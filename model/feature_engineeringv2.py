...
def build_lic_features(df_lic: pd.DataFrame) -> pd.DataFrame:
    ...
    sex_col = next((c for c in ["sexe", "genre", "sex", "gender"] if c in df.columns), None)
    ...
    if age_col is not None:
            ...
            return pd.Series(
                {
                    "age_mean": age_mean,
                    "age_std": age_std,
                    "nb_lt14": x.loc[x["is_jeune_lt14"] == 1, "licences_annuelles"].sum(),
                    "nb_14_17": x.loc[x["is_jeune_14_17"] == 1, "licences_annuelles"].sum(),
                    "nb_18_34": x.loc[x["is_adulte_18_34"] == 1, "licences_annuelles"].sum(),
                    "nb_35_49": x.loc[x["is_adulte_35_49"] == 1, "licences_annuelles"].sum(),
                    "nb_50p": x.loc[x["is_senior_50p"] == 1, "licences_annuelles"].sum(),
                }
            )
        ...
        for c in tranche_cols:
            out[f"part_{c.replace('nb_', '')}"] = safe_div(out[c], out["licences_annuelles"])
    ...
    out["licences_annuelles_lag1"] = out.groupby("code_sport")["licences_annuelles"].shift(1)
    out["licences_annuelles_lag2"] = out.groupby("code_sport")["licences_annuelles"].shift(2)
    ...
    def _block(year: int) -> pd.DataFrame:
        return (
            df_sport[
                [
                    "code_sport", "sport",
                    f"{year}_or", f"{year}_argent", f"{year}_bronze",
